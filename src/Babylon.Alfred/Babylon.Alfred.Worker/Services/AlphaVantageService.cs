...
public class AlphaVantageService
{
    ...
    public AlphaVantageService(HttpClient httpClient, ILogger<AlphaVantageService> logger)
    {
    ...
    public async Task<decimal?> GetCurrentPriceAsync(string ticker)
    {
        try
        {
            ...
            _logger.LogInformation("Fetching price for {Ticker} from Alpha Vantage at {Timestamp}", ticker, DateTime.UtcNow);
            ...
            if (jsonDoc.RootElement.TryGetProperty("Global Quote", out var globalQuote))
            {
                if (globalQuote.TryGetProperty("05. price", out var priceElement))
                {
                    ...
                    {
                        _logger.LogInformation("Successfully fetched price for {Ticker}: {Price} at {Timestamp}", ticker, price, DateTime.UtcNow);
                        return price;
                    }
                }
            }
            ...
            {
                _logger.LogWarning("Alpha Vantage API error for {Ticker}: {Error}", ticker, errorMessage.GetString());
                return null;
            }

            if (jsonDoc.RootElement.TryGetProperty("Note", out var note))
            {
                _logger.LogWarning("Alpha Vantage API rate limit notice for {Ticker}: {Note}", ticker, note.GetString());
                return null;
            }

            _logger.LogWarning("Unexpected response format for {Ticker}", ticker);
            return null;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error fetching price for {Ticker} from Alpha Vantage", ticker);
            return null;
        }
    }
}
